 ...
 */
public class SubscribeWriter extends PacketWriter
{
    Logger logger = Logger.getLogger(SubscribeWriter.class);

    public SubscribeWriter(Packet packet)
    {
        super(packet);
    }
    ...
    protected void writePayload(OutputStream out) throws IOException
    {
        ...
        for(SubscribeTopicFilter topicFilter : getPacket().getTopicFilters())
        {
            ...
            byte qos = 0;
            if(topicFilter.getQos() == 1)
                qos = Util.setBitOn(qos, 0);
            if(topicFilter.getQos() == 2)
                qos = Util.setBitOn(qos, 1);
            writeByte(out, qos, "Request Qos - " + index);
            index++;
        }
        return;
    }
